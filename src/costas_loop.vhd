		...
		IF clk'EVENT AND clk = '1' THEN 
			...
			IF enable = '1' THEN
				...
				IF tclk = '1' THEN 
					...
					rx_sin_dump 	<= resize(shift_right(signed(rx_sin_acc), SINUSOID_W + 2), ACC_W); -- +2: compensates for full-scale LUT (ref 2x * rx 2x = 4x product)
					rx_cos_dump 	<= resize(shift_right(signed(rx_cos_acc), SINUSOID_W + 2), ACC_W); -- +2: compensates for full-scale LUT (ref 2x * rx 2x = 4x product)
